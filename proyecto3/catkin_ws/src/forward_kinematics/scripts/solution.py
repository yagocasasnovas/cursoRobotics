...
def get_value(name,joint_values):
	
	for idx, nm in enumerate(joint_values.name):
		if nm == name:
			return joint_values.position[idx]
...
def convert_link_to_transform(joint,index,joint_value,prev):
	...
	#transform = tf.transformations.concatenate_matrices(transform,prev)
